...
mp.grid(linestyle=':')
mp.plot(times[:200], noised_sigs[:200], color='dodgerblue', label='Signal')
mp.tight_layout()
mp.legend()
...
# fund_freq = freqs[noised_pow >= 1111]
# fund_freq = fund_freq[fund_freq > 0]
...
mp.tick_params(labelsize=10)
mp.grid(linestyle=':')
mp.plot(times[:200], filter_sigs[:200],
